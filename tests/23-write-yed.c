...
int main(void)
{
	...
	TEST_ASSERT(doc);
	TEST_ASSERT(cyberiada_read_sm_document(doc, "samples/ostranna-player.graphml",
										   cybxmlUnknown, CYBERIADA_FLAG_NO) ==
				CYBERIADA_NO_ERROR);
	TEST_ASSERT(doc->format);
	TEST_ASSERT(strcmp(doc->format, "yEd Ostranna") == 0);

	TEST_ASSERT(cyberiada_write_sm_document(doc, "23-out.graphml",
											cybxmlYEDOstranna,
											CYBERIADA_FLAG_NO) ==
				CYBERIADA_NO_ERROR);
	TEST_ASSERT(test_compare_files("23-out.graphml",
								   "good/23-write-yed.graphml") == 0);

	/* the written document keeps the dialect and the whole graph */
	written = cyberiada_new_sm_document();
	TEST_ASSERT(written);
	TEST_ASSERT(cyberiada_read_sm_document(written, "23-out.graphml",
										   cybxmlUnknown, CYBERIADA_FLAG_NO) ==
				CYBERIADA_NO_ERROR);
	TEST_ASSERT(written->format);
	TEST_ASSERT(strcmp(written->format, "yEd Ostranna") == 0);
